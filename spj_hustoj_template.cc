...
int spj(FILE *input, FILE *std_output, FILE *user_output);
...
int main(int argc, const char *args[]) {
    FILE *input = NULL, *user_output = NULL, *std_output = NULL;
    int result;
    if (argc != 4) {
        printf("Usage: spj x.in x.out user.out\n");
        return ERROR;
    }
    input       = fopen(args[1], "r");
    std_output  = fopen(args[2], "r");
    user_output = fopen(args[3], "r");
    if (input == NULL || user_output == NULL || std_output == NULL) {
        printf("Failed to open output file\n");
        close_file(input);
        close_file(user_output);
        close_file(std_output);
        return ERROR;
    }

    result = spj(input, std_output, user_output);
    printf("result: %d\n", result);
